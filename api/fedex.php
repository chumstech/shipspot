<?php
	  ...
	  if($userObj->is_privilege_discount == 1 and $userObj->user_type !=  2){
		...
		if(!is_array($rateData)){
			$insertClause = array('user_id' =>$userObj->id,'api_name' => 'Fedex','response_short' => 'Fail','response' => json_encode(array("No Response Return")));
		}else{
			$insertClause = array('user_id' =>$userObj->id,'api_name' => 'Fedex','response_short' => 'Success','response' => json_encode($rateData));
		}
